...
public class PromotionController implements PromotionBLService{
	...
	public ResultMessage addPromotion(PromotionVO pvo) {
		...
		try{
			...
			PromotionPO ppo = new PromotionPO( pvo.getPromotionName(), pvo.getContent(), pvo.getStartTime(), pvo.getStopTime(), converse(pvo.getPromotionType()), pvo.getID());
			...
			if(listPromotion == null){
				...
				for(int i = 0; i<listPromotion.size(); i++){
					tmp = new PromotionVO( listPromotion.get(i).getPromotionName(), listPromotion.get(i).getContent(), listPromotion.get(i).getStartTime(),
							listPromotion.get(i).getStopTime(), converse(listPromotion.get(i).getPromotionType()), listPromotion.get(i).getID());
					list.add(tmp);
				}
			}
	...
	public ResultMessage addFullCutPromotion(FullCutPromotionVO fvo) {
		...
		try{
			...
			FullCutPromotionPO fpo = new FullCutPromotionPO( fvo.getPromotionName(), fvo.getContent(), fvo.getStartTime(), fvo.getStopTime(), converse(fvo.getPromotionType()), fvo.getID(), fvo.getEvery(), fvo.getCut());//....
	...
	public ResultMessage addDiscountPromotion(DiscountPromotionVO dvo) {
		...
		try{
			...
			DiscountPromotionPO dpo = new DiscountPromotionPO( dvo.getPromotionName(), dvo.getContent(), dvo.getStartTime(), dvo.getStopTime(), converse(dvo.getPromotionType()), dvo.getID(), dvo.getDiscount());//....
	...
	public PromotionVO searchPromotionPresent(int id, Timestamp presentTime) {
		...
		try{
			...
			if(available.size() == 1){
				pvo = new PromotionVO(available.get(0).getPromotionName(), available.get(0).getContent(),  available.get(0).getStartTime(), available.get(0).getStopTime(), converse(available.get(0).getPromotionType()), available.get(0).getID());
				return pvo;
				...
				if(one == "2" && two != "2"){
					...
					pvo = new PromotionVO(available.get(1).getPromotionName(), available.get(1).getContent(),  available.get(1).getStartTime(), available.get(1).getStopTime(), converse(available.get(1).getPromotionType()), available.get(1).getID());
				    return pvo;
				}else{
					return null;
				}
